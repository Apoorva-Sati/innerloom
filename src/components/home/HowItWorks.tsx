          ...
          {steps.map((step, i) => (
            <div key={step.num} className="flex flex-col md:flex-row items-start flex-1 gap-4">
              <div className="flex flex-col items-center">
                <div className="w-10 h-10 rounded-full border-2 border-[var(--color-terra)] flex items-center justify-center text-[var(--color-terra)] font-display text-lg font-medium flex-shrink-0">
                  {step.num}
                </div>
                {i < steps.length - 1 && (
                  <div className="hidden md:block w-px h-0 border-t border-dashed border-[var(--color-peach)] absolute" />
                )}
              </div>
              <div>
                <h3 className="text-lg text-[var(--color-teal)] mb-1">{step.title}</h3>
                <p className="text-sm text-[var(--color-brown-mid)] leading-relaxed">{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
